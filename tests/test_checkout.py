...
class TestCheckout:
    ...
    def go_to_step_one(self, product_key="bike_light"):
        """Helper: add an item, open the cart, then start checkout."""
    ...
    def go_to_step_two(self, first_name="John", last_name="Doe", postal="12345"):
        """Helper: fill the form and continue to step 2."""
    ...
    @allure.story("Step 1 - Display checkout form")
    def test_step_one_displayed(self):
        self.go_to_step_one()

        with allure.step("Verify checkout form is displayed"):
            assert self.step_one.is_checkout_step_one_displayed()

    @pytest.mark.negative
    @allure.story("Step 1 - Missing required fields")
    ...
    def test_step_one_missing_fields(self, first_name, last_name, postal, expected_error):
        ...
        with allure.step("Submit form with a missing field"):
            self.step_one.enter_information(first_name, last_name, postal)
            self.step_one.click_continue()
    ...
    @pytest.mark.positive
    @allure.story("Step 1 - Cancel back to Cart")
    def test_step_one_cancel(self):
        ...
        with allure.step("Verify return to Cart"):
    ...
    # ─────────────────────────────────────────
    @pytest.mark.positive
    @allure.story("Step 2 - Display order summary")
    def test_step_two_displayed(self):
        self.go_to_step_one()
        self.go_to_step_two()

        with allure.step("Verify order summary is displayed"):
            assert self.step_two.is_checkout_step_two_displayed()

    @pytest.mark.positive
    @allure.story("Step 2 - Verify product name and price")
    def test_step_two_item_info(self):
        ...
        with allure.step(f"Verify name: '{expected_name}'"):
            assert self.step_two.get_item_name() == expected_name

        with allure.step(f"Verify price: '{expected_price}'"):
            assert self.step_two.get_item_price() == expected_price

    @pytest.mark.positive
    @allure.story("Step 2 - Verify correct total")
    def test_step_two_total_correct(self):
        self.go_to_step_one("bike_light")
        self.go_to_step_two()

        with allure.step("Verify subtotal + tax equals total"):
            ...
            assert round(subtotal + tax, 2) == total, \
                f"Incorrect total: {subtotal} + {tax} != {total}"

    @pytest.mark.positive
    @allure.story("Step 2 - Cancel back to Inventory")
    ...
    def test_complete_order(self):
        ...
        with allure.step("Verify order is complete"):
            assert self.step_two.is_order_complete(), \
                "Order confirmation screen is not displayed"
